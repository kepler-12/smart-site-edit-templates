    ...
    try {      
      ...
      templates.nodes.forEach(template => {
        ...
        if (template.name && template.vue) {
          ...
          console.log('template', `/templates/${resource.name}/${template.name}-${template.id}.vue`);
          const sync = fs.writeFileSync(`./templates/${resource.name}/${template.name}-${template.id}.vue`, stringTemplate)
          console.log(sync)
        }
      })
    }
